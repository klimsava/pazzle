...
module.exports = app => {
    ...
    app.post('/login', async (req, res) => {
        try {
            ...
            if (user != void (0) && bcrypt.compareSync(req.body.password, user.password)) {
                ...
                res.status(200).send({ message: "Пользователь успешно зашел." });
            } else res.status(400).send({ message: "Пароль не верный или пользователь не существует." });
        } catch (e) {
            console.error("E, login,", e);
            res.status(500).send({ message: "какая-то ошибка" });
        }
    });
    ...
    app.post('/register', async (req, res) => {
        try {
            let user = await UsersModel.findOne({ username: { $regex: _.escapeRegExp(req.body.username), $options: "i" } }).lean().exec();
            if (user != void (0)) return res.status(400).send({ message: "Пользователь уже существует" });
    ...
    app.post('/logout', (req, res) => {
        ...
        res.status(200).send({ message: "Выйти из системы." });
    })
};